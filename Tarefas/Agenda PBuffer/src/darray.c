...
uint32_t *DArrayGetArg(void *darray) { return ARENA_GET(uint32_t, darray); }
uint32_t *DArrayGetElemSize(void *darray) {
  return OFFSET_BY(ARENA_GET(uint32_t, darray), 4U);
}

uint32_t const *DArraySize(void *darray) { return DArrayGetUsed(darray); }
...
void *DArrayPush(void *darray) {
  ...
  void *element = OFFSET_BY(ARENA_GET(void, darray), *DArraySize(darray));
...
void *DArrayAt(void *darray) {
  if (*DArrayGetArg(darray) < *DArrayGetUsed(darray)) {
    return OFFSET_BY(ARENA_GET(void, darray), *DArrayGetArg(darray));
  }
  return NULL;
}
...
static uint32_t *DArrayGetUsed(void *darray) {
  return OFFSET_BY(ARENA_GET(uint32_t, darray), 8U);
}
static uint32_t *DArrayGetCapacity(void *darray) {
  return OFFSET_BY(ARENA_GET(uint32_t, darray), 12U);
}
